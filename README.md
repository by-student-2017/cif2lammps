...
- In [1], RASPA is used, but GCMC in Lammps requires input of chemical potential (Note: [lammps] As an alternative to specifying mu directly, the ideal gas reservoir can be defined by its pressure P using the pressure keyword, in which case the user-specified chemical potential is ignored.). I use the HOMO energy of MOPAC (the chemical potential at 0 K corresponds to the Fermi energy in DFT. It is important to note that MOPAC parameters are optimized for data at 25 degrees or room temperature).
