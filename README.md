...
- The original code does not support COF etc. In the MIL series, non-zero charges are listed in the cif file. When charges are taken into account in UFF or UFF4MOF, the initial structure shrinks abnormally, so the Coulomb force considering the charges is probably being calculated twice. To solve this, it is necessary to rewrite the parameters of UFF or UFF4MOF.
