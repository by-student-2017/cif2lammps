...
- Unless it is stated that the parameters have been optimized by adding charges to UFF, UFF4MOF, or Dreiding and taking into account Coulomb forces, it is best not to trust reports that show formulas calculated by adding Coulomb forces. Adding Coulomb forces that take charge into account to such a force field means that the Coulomb forces are being calculated twice.
- Of course, there is no disagreement that the accuracy of predictions of adsorption phenomena affected by charge (Coulomb forces), such as CO2, is improved when parameters are optimized taking these issues into account. That is, if the parameters have been optimized.
