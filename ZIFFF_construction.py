...
class ZIFFF(force_field):
    ...
    def enumerate_dihedrals(self):
        ...
        for e in SG.edges(data=True):
            ...
            dihedral_list = [(SG.nodes[p[0]]['force_field_type'],fft_j,fft_k,SG.nodes[p[1]]['force_field_type']) for p in il_pairs]
    ...
    def enumerate_impropers(self):
        ...
        for n in SG.nodes(data=True):
            ...
            if len(nbors) == 3:
                ...
                nbors = sorted([(m,SG.nodes[m]['force_field_type']) for m in nbors], key=lambda x:x[1])
